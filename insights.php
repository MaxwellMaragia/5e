<?php
    ...
    $sql = "SELECT * FROM news WHERE state=1 ORDER BY id DESC $limit ";
    $query = mysqli_query($obj->con,$sql);
    ...
    if($last != 1){
        ...
        if ($pagenum > 1) {
            $previous = $pagenum - 1;
            $paginationCtrls .= ' &nbsp; &nbsp; <li style="margin-top:-5px;"><a href="./insights?pn='.$previous.'" rel="next" class="page-prev"><span class="fa fa-arrow-left"></span></a></li> ';
            //render clickable number links that should appear on the left of the target page number

            for ($i = $pagenum-4; $i < $pagenum; $i++){
                if($i > 0){
                    $paginationCtrls .='<li><a href="./insights?pn='.$i.'" class=" ">'.$i.'</a></li> &nbsp; ';
                }
            }
        ...
        {
            $next = $pagenum + 1;
            $paginationCtrls .= ' &nbsp; &nbsp; <li style="margin-top:-5px;"><a href="./insights?pn='.$next.'" rel="next" class="page-next"><span class="fa fa-arrow-right"></span></a></li> ';
        }
    }
...
<?php
$sql = "SELECT * FROM news WHERE media_type='image' AND state=1 ORDER BY ID DESC limit 5";
...
if(mysqli_num_rows($exe)>0)
{
    ...
    ?>
    <section class="wow fadeIn no-padding margin-100px-top">
                    ...
                    <div class="swiper-blog white-move">
                        <div class="swiper-wrapper">
                            ...
                            while($row = mysqli_fetch_assoc($exe))
                            {
                                ...
                                $get_cat = mysqli_fetch_assoc($execute);
                                $category = $get_cat['name'];
                              ?>
                                <!-- start blog slider item -->
                                <div class="swiper-slide width-50 md-width-60 sm-width-80 xs-width-100">
                                    <div class="cover-background one-second-screen" style="background:url(management/<?=$row['media']?>)">
                                        ...
                                        <div class="display-table width-100 height-100 position-relative">
                                            <div class="display-table-cell vertical-align-middle text-center">
                                                <div class="col-lg-8 col-md-10 col-sm-10 col-xs-12 center-col slide-content">
                                                    <a href="insights?cid=<?=$cid?>" class="text-white text-extra-small text-uppercase alt-font font-weight-600 highlight-bg-text letter-spacing-1 bg-black"><?=$category?></a>
